...
void main() {
  ...
  group('extractExePath — unquoted paths', () {
    test('extracts simple unquoted path with args', () {
      expect(
        extractExePath(r'C:\chrome.exe --flag'),
        r'C:\chrome.exe',
      );
    });
    ...
    test('extracts path when args use %1 placeholder', () {
      expect(
        extractExePath(r'C:\chrome.exe %1'),
        r'C:\chrome.exe',
      );
    });
    ...
    test('path without .exe falls back to space split', () {
      expect(
        extractExePath(r'C:\launcher --go'),
        r'C:\launcher',
      );
    });
  ...
  group('extractExePath — intermediate .exe directory', () {
    ...
    test('quoted path with intermediate .exe directory is handled correctly', () {
      expect(
