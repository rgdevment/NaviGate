...
void main() {
  group('WinPipeServer event buffering (P1.6)', () {
    test(
      'event emitted before first listen is buffered and flushed on listen',
      () async {
    ...
    test(
      'buffer is not re-flushed when a second listener subscribes',
      () async {
        final server = WinPipeServer();
  ...
  group('migrateDirIfNeeded (P1.7)', () {
    ...
    test('no-op when newDir already holds user data (browsers.json)', () {
      final oldDir = Directory('${tempDir.path}/old')..createSync();
      File('${oldDir.path}/browsers.json').writeAsStringSync('old');
      final newDir = Directory('${tempDir.path}/new')..createSync();
      File('${newDir.path}/browsers.json').writeAsStringSync('new');

      migrateDirIfNeeded(oldDir, newDir);

      expect(oldDir.existsSync(), isTrue);
      expect(File('${newDir.path}/browsers.json').readAsStringSync(), 'new');
    });

    test('migrates into existing empty newDir (startup creates it before '
        'migration runs)', () {
      final oldDir = Directory('${tempDir.path}/old')..createSync();
      File('${oldDir.path}/browsers.json').writeAsStringSync('{}');
      File('${oldDir.path}/rules.json').writeAsStringSync('[]');
      final newDir = Directory('${tempDir.path}/new')..createSync();

      migrateDirIfNeeded(oldDir, newDir);

      expect(File('${newDir.path}/browsers.json').existsSync(), isTrue);
      expect(File('${newDir.path}/rules.json').existsSync(), isTrue);
      expect(oldDir.existsSync(), isFalse);
    });

    test('existing destination files are kept, missing ones are copied', () {
      final oldDir = Directory('${tempDir.path}/old')..createSync();
      File('${oldDir.path}/browsers.json').writeAsStringSync('{}');
      File('${oldDir.path}/navigate.log').writeAsStringSync('old-log');
      final newDir = Directory('${tempDir.path}/new')..createSync();
      File('${newDir.path}/navigate.log').writeAsStringSync('current-log');

      migrateDirIfNeeded(oldDir, newDir);

      expect(File('${newDir.path}/browsers.json').existsSync(), isTrue);
      expect(
        File('${newDir.path}/navigate.log').readAsStringSync(),
        'current-log',
      );
    });
    ...
    test('copyDirRecursive copies nested files and deletes source', () {
      ...
      expect(
        File('${dstDir.path}/browsers.json').readAsStringSync(),
        '{"browsers":[]}',
      );
      expect(File('${dstDir.path}/icons/chrome.png').readAsBytesSync(), [
        1,
        2,
        3,
      ]);
    ...
    test('migrateDirIfNeeded preserves data: content matches after move', () {
      ...
      File(
        '${oldDir.path}/rules.json',
      ).writeAsStringSync('[{"domain":"x.com"}]');
